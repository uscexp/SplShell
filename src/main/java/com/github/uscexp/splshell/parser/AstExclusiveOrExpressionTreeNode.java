 ...
 * 
 */
public class AstExclusiveOrExpressionTreeNode<V>
		extends AstCalculationExpressionTreeNode<V> {

	public AstExclusiveOrExpressionTreeNode(String rule, String value) {
		super(rule, value);
	}
	...
	protected void interpretAfterChilds(Long id)
		...
		if (isExclusiveOrExpressionPattern()) {
			process(id, AstAndExpressionTreeNode.class, AstCharLiteralTreeNode.class);
		}
	}

	private boolean isExclusiveOrExpressionPattern() {
		boolean result = false;
		if (getChildren().size() >= 2) {
			AstTreeNode<V> treeNode1 = getChildren().get(0);
			AstTreeNode<V> treeNode2 = getChildren().get(1);
			if (treeNode1 instanceof AstAndExpressionTreeNode
					&& getValue().indexOf("^") > 0
					&& treeNode2 instanceof AstAndExpressionTreeNode) {
				result = true;
			}
		}
		return result;
